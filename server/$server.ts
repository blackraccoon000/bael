...
type BlobToFile<T extends AspidaMethodParams> = T["reqFormat"] extends FormData
  ? {
      [P in keyof T["reqBody"]]: Required<T["reqBody"]>[P] extends
        | Blob
        | ReadStream
        ? MultipartFile
...
> = (req: RequestParams<T> & U) => ServerResponse<T>;

type ServerHandlerPromise<
  T extends AspidaMethodParams,
  U extends Record<string, unknown> = {}
> = (req: RequestParams<T> & U) => Promise<ServerResponse<T>>;

type AddedHandler<T, R extends Record<string, unknown>> = T extends (
  req: infer U,
  ...args: infer V
) => infer W
  ? (req: U & Partial<R>, ...args: V) => W
  : never;

export type ServerHooks<R extends Record<string, unknown> = {}> = {
  onRequest?:
    | AddedHandler<onRequestHookHandler, R>
    | AddedHandler<onRequestHookHandler, R>[];
  preParsing?:
    | AddedHandler<preParsingHookHandler, R>
    | AddedHandler<preParsingHookHandler, R>[];
  preValidation?:
    | AddedHandler<preValidationHookHandler, R>
    | AddedHandler<preValidationHookHandler, R>[];
  preHandler?:
    | AddedHandler<preHandlerHookHandler, R>
    | AddedHandler<preHandlerHookHandler, R>[];
};
...
> =
  ...
  | {
      validators?: {
        [Key in keyof RequestParams<T>]?: z.ZodType<RequestParams<T>[Key]>;
      };
...
const methodToHandler =
  (methodCallback: ServerHandler<any, any>): RouteHandlerMethod =>
  (req, reply) => {
